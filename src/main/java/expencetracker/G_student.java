...
public class G_student {
    ...
    public void addGrade(String subjectCode, char grade){
        ...
        if(gradeMap.containsKey(key)){
            List<String> gradesList = gradeMap.get(key); // Hent eksisterende karakterliste
            gradesList.add(String.valueOf(grade)); // Legg til den nye karakteren
            gradeMap.put(key, gradesList); // Legg den oppdaterte listen tilbake i gradeMap
            System.out.println("Grade added for " + studentName + " in subject " + subjectCode);
            // var det eneste som var i if setningen tidligere - System.out.println("Student have already added grade.");
        } else {
